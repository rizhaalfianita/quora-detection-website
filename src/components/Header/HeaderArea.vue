...
<template>
  <header class="flex w-full dark:bg-boxdark dark:drop-shadow-none">
      ...
      <div class="flex items-center gap-2 sm:gap-4 lg:hidden">
        ...
        >
          <span class="relative block h-5.5 w-5.5 cursor-pointer">
            ...
            <span class="du-block absolute right-0 h-full w-full">
              <span
                class="absolute left-0 top-0 my-1 block h-0.5 w-full rounded-sm bg-black delay-300 duration-200 ease-in-out dark:bg-white"
                :class="{ '!h-0 delay-[0]': !isSidebarOpen }"
              ></span>
              <span
                class="delay-400 absolute left-0 top-1.5 block h-0.5 w-full my-1 rounded-sm bg-black duration-200 ease-in-out dark:bg-white"
                :class="{ '!h-0 delay-200': !isSidebarOpen }"
              ></span>
              <span
                class="absolute left-0 top-3 my-1 block h-0.5 w-full rounded-sm bg-black delay-300 duration-200 ease-in-out dark:bg-white"
                :class="{ '!h-0 delay-300': !isSidebarOpen }"
              ></span>
            </span>
          </span>
        </button>
        ...
        <router-link class="block flex-shrink-0 lg:hidden" to="/">
          <img src="@/assets/images/logo/quora-logo.svg" alt="Logo" class="w-25" />
        </router-link>
      </div>
